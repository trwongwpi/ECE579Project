...
def train(model, device, config, epochs=5, batch_size=1, save_cp=True, log_step=20, img_scale=0.5, callbacks= None):
...
def get_args(**kwargs):
    ...
    parser.add_argument('-train_label_path', dest='train_label', type=str, default='/content/pytorch-YOLOv4/data/roboflow/train/_annotations.txt', help="train label path")
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    if cfg.use_darknet_cfg:
        ...
        pretrained_dict = torch.load('/content/pytorch-YOLOv4/Yolov4_epoch10.pth', map_location=torch.device('cuda'))
    ...
    try:
        ...
        train(model=model,
              ...
              device=device, callbacks=[lambda: pruner.apply(model), lambda: quantizer.apply(model,True)])
